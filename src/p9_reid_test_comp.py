...
from src.p8_facr_test_info import P8FacrTestInfo
from src.p9_reid_test_info import P9ReidTestInfo
...
class P9ReidTestComponent(BasedStreamComponent):

    def __init__(self, shared_memory, config_path: str):
        super().__init__(shared_memory)
        self.config: P9ReidTestInfo = P9ReidTestInfo(ConfigKit.load(config_path))
        self.tracker: BytetrackHelper = BytetrackHelper(self.config.stream_mot_config)  # 追踪器
        self.reid_helper: ReidHelper = ReidHelper(self.config.reid_helper_config, self.reid_callback)
        # key: obj_id
        # value: {"per_id": 1, "score": 0, "ltrb": (0,0,0,0)}
        self.item_dict: Dict[int, dict] = {}
        self.cam_id = 0
    ...
    def on_handle_stream(self, idx, frame, input_det) -> object:
        ...
        for item in mot_result:
            ...
            if not self.item_dict.__contains__(obj_id):
                data = {"per_id": 1, "score": 0}
                self.item_dict[obj_id] = data
        ...
        for obj_id, item in self.item_dict.items():
            ...
            image = ImgKit.crop_img(self.frames[0], ltrb)
            self.reid_helper.try_send_reid(current_id, image, obj_id, self.cam_id)
    ...
    def on_update(self):
        ...
        current_id = self.frame_id_cache[0]
        # face tick
        self.reid_helper.tick(current_id)
...
def create_process(shared_memory, config_path: str):
    comp = P9ReidTestComponent(shared_memory, config_path)  # 创建组件
    try:
        comp.start()  # 初始化
        # 初始化结束通知
        ...
        logger.error(f"P9ReidTestComponent: {e}")
