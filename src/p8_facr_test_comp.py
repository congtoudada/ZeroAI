...
class P8FacrTestComponent(BasedStreamComponent):

    def __init__(self, shared_memory, config_path: str):
        super().__init__(shared_memory)
        self.config: P8FacrTestInfo = P8FacrTestInfo(ConfigKit.load(config_path))
        self.tracker: BytetrackHelper = BytetrackHelper(self.config.stream_mot_config)  # 追踪器
        self.face_helper: FaceHelper = FaceHelper(self.config.face_helper_config, self.face_callback)
        # key: obj_id
...
def create_process(shared_memory, config_path: str):
    comp = P8FacrTestComponent(shared_memory, config_path)  # 创建组件
    try:
        comp.start()  # 初始化
        # 初始化结束通知
        ...
        logger.error(f"P8FacrTestComponent: {e}")
