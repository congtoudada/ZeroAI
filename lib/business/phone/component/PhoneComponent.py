...
class PhoneComponent(BasedMultiMOTComponent):
    ...
    #         # 构建请求数据
    #         data = {
    #             "query_directory_or_id": temporary_warning_path,
    ...
    def on_execute(self, all_bboxes, object_bboxes):   
        ...
        if self.timing_record is None or (now - self.timing_record) >= delta:
            ...
            # print(f"调试: Timing Operation will be available after {delta - (now - self.timing_record)}.")
            pass
        ...
        max_gap = timedelta(seconds=0.5)
        # 遍历所有被检测到的对象的包围框
        for id in object_bboxes.keys():
            # 如果对象ID不在已报警的状态记录中
            if id not in self.state:  # 没报过警
                # 如果没有检测记录或者距离上次检测到现在的时间间隔超过了设定的最大间隔
                if id not in self.det_record or w_now - self.det_record[id] > max_gap:
                    # 重置计数器为1并记录当前时间
                    self.counter[id] = 1
                    self.det_record[id] = w_now
                # 如果距离上次检测到现在的时间间隔小于等于设定的最大间隔
                elif w_now - self.det_record[id] <= max_gap:  # 连续检测到一个人
                    # 增加对应ID的计数器
                    self.counter[id] += 1
                    self.det_record[id] = w_now
                    # 如果连续检测到的次数超过了10次，则触发报警
                    if self.counter[id] > 10:  # 连续检测到了k次进行报警
                        ...
                        # 保存图片并获取路径和图片本身
                        image_path, img = self.on_save_img(object_bboxes[id], id, self.config.warning_path)
                        # 发送警告信息！！！！！！！！！！！！！！！！！！！
