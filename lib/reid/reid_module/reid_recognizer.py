...
# )
import OSNet.scripts.torchreid
from OSNet.scripts.torchreid.models import  build_model
from OSNet.scripts.torchreid.optim import (build_optimizer, build_lr_scheduler)  # pzy
from OSNet.scripts.torchreid.data.datamanager import ImageDataManager  # pzy
from OSNet.scripts.torchreid.engine import ImageSoftmaxEngine  # pzy
from OSNet.scripts.torchreid.utils import (
    Logger, check_isfile, set_random_seed, collect_env_info,
    resume_from_checkpoint, load_pretrained_weights, compute_model_complexity
)

from OSNet.scripts.default_config import (
    imagedata_kwargs, optimizer_kwargs, videodata_kwargs, engine_run_kwargs,
    get_default_config, lr_scheduler_kwargs
)
...
class Reid:
    def __init__(self,config_path):
        ...
        print(config)
        print(config.keys())
        # config = ConfigKit.load(config_path)
        self.parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        self.parser.add_argument('opts', default=None, nargs=argparse.REMAINDER,
        ...
        self.parser.add_argument('--config-file', type=str, default=config['config_file'],
                                 help='path to config file')
        self.parser.add_argument('--transforms', type=str, nargs='+', default=['random_flip', 'random_erase'],
                                 help='data augmentation')
        self.parser.add_argument('--root', type=str, default=config['root'], help='path to data root')
        self.parser.add_argument('--model_weights', type=str,
                                 default=config['model']['load_weights'],
                                 help='path to model weights')
        self.parser.add_argument('--test_evaluate', action='store_true', default=True,
...
if __name__ == '__main__':
    config_path = 'conf/algorithm/reid/mode_config.yaml'
    reid_instance = Reid(config_path)
    reid_instance.run()
